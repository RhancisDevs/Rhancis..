...
fs.writeFileSync("./ATF_Config.json", JSON.stringify(global.fca.data, null, "\t"))
  return process.exit(1)
}
try {
      var langfile = JSON.parse(fs.readFileSync(__dirname + "/languages/languages.json", 'utf-8'));
var lang
    switch (require("../../ATF_Config.json").languages) {
      ...
      case "jp": lang = langfile.jp.index;
        break;
      default: {
      log.warn("warn", "Only 4 Kinds of Languages ​​EN, VI, TH & JP Currently Supported, Self Fix By Finding And Deleting ATF_Config.json File")
      process.exit(0)
      }
    }
  } catch(e) {
...
global.fca = new Object({
     ObjFcaConfig: require("../../ATF_Config.json"),
     languages: lang
})
...
axios.get(`https://uptime-api.0xanupx0.repl.co/uptime?link=${UrlRepl}`)
		logger(global.fca.languages.uptime + UrlRepl, '[ CHAND - TRICK  ]');
	}
	catch {
		logger(global.fca.languages.erroUptime, '[ CHAND - TRICK  ]');
  }
}
...
function buildAPI(globalOptions, html, jar) {
    ...
    logger(`${global.fca.languages.loginSu}${userID}`, "[ CHAND - TRICK  ]");
    ...
    if (oldFBMQTTMatch) {
        ...
        logger(`${global.fca.languages.region}${region}`, "[ CHAND - TRICK  ]");
    } else {
...
logger(global.fca.languages.login, "[ CHAND - TRICK  ]");
        return utils
            ...
            .then(function(res) {
                ...
                if (headers.location.indexOf('https://www.facebook.com/checkpoint/') > -1) {
                    logger(global.fca.languages.errorFa, "[ CHAND - TRICK  ]");
...
require('dotenv').config({ path: './../.env' });
                ...
                var ans = makeid(49)
                    process.env["FBKEY"] = ans;
                        fs.writeFile('./../.env', `FBKEY=${ans}`, function (err) {
                            if (err) {
                            logger(global.fca.languages.errorEnv, "[ CHAND - TRICK  ]");
                    }
                else logger(global.fca.languages.env,"[ CHAND - TRICK  ]")
        }); 
    }
    catch (e) {
        console.log(e);
        logger(global.fca.languages.errorPassRandom, "[ CHAND - TRICK  ]");
    }
}
    ...
    if (process.env['FBKEY']) {
        try {
            ...
            if (appState.includes('[')) {
                logger(global.fca.languages.appstateDec, '[ CHAND - TRICK  ]');
            } else {
                try {
                    appState = JSON.parse(appState);
                    var StateCrypt = require('./StateCrypt');
                    appState = StateCrypt.decryptState(appState, process.env['FBKEY']);
                    logger(global.fca.languages.appstate, '[ CHAND - TRICK  ]');
                }
                catch (e) {
                    logger(global.fca.languages.appstateError, '[ CHAND - TRICK  ]');
                }
            }
        }
        catch (e) {
    ...
    try {
        ...
        try {
            ...
            return logger(global.fca.languages.appstateError, '[ CHAND - TRICK  ]')
        }
    }
      }
    try { 
    ...
    mainPromise
        .then(function() {
            logger(global.fca.languages.oklogin, "[ CHAND - TRICK  ]");
                    //!---------- Auto Check, Update START -----------------!//
                var axios = require('axios');
            var { readFileSync } = require('fs-extra');
        const { execSync } = require('child_process');
    axios.get('https://raw.githubusercontent.com/Mrchandu7/chand-trick/main/package.json').then(async (res) => {
        const localbrand = JSON.parse(readFileSync('./node_modules/chand-trick/package.json')).version;
            if (localbrand != res.data.version) {
                        log.warn("UPDATE > ",`${global.fca.languages.newVersion}${JSON.parse(readFileSync('./node_modules/chand-trick /package.json')).version} => ${res.data.version}`);
                        log.warn("UPDATE > ",`${global.fca.languages.autoUpdate}`);
                            try {
                                execSync('npm install chand-trick @latest', { stdio: 'inherit' });
                                logger(global.fca.languages.okUpdate,"UPDATE")
                                logger(global.fca.languages.restart, '[ CHAND - TRICK  ]');
                    ...
                    logger(`${global.fca.languages.checkVersion}` + localbrand + ' !', "[ CHAND - TRICK  ]");      
                         logger(global.fca.languages.chucAdmin, "[ CHAND - TRICK  ]");
